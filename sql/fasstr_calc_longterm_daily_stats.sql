...
drop function if exists fasstr_calc_longterm_daily_stats(date[], numeric[], boolean);
drop function if exists fasstr_calc_longterm_daily_stats(text, boolean);
...
$$
  ...
  select fasstr_calc_longterm_daily_stats(dates, values) from flowdata
$$
LANGUAGE 'sql'
;
